...
class DispatchLLM:
    ...
    def __init__(self, *, preextract, ground=None, finalize=None,
                 missing='{"missing_characters":[],"missing_places":[]}'):
        self.preextract = preextract  # str | callable(user)->str
        self.ground = ground
        self.finalize = finalize or '{"groups":[],"canon_note":"主线","playable_archetypes":[],"out_of_continuity":[]}'
        self.missing = missing
        self.calls = {"preextract": 0, "ground": 0, "missing": 0, "finalize": 0}
...
class FakeGrok:
    def __init__(self, text="演员A 饰演 甄嬛，演员B 饰演 皇后", citations=None):
        self.text = text
        self.citations = citations if citations is not None else [{"url": "https://example.com/source"}]
        self.calls = []

    async def web_search(self, query, max_tokens=2048):
        self.calls.append(query)
        return SimpleNamespace(text=self.text, citations=self.citations)
...
@pytest.mark.asyncio
async def test_gather_via_grok_fanout_runs_2_complementary_queries():
...
@pytest.mark.asyncio
async def test_loose_pipeline_without_live_search_uses_one_fallback_compile():
    """loose 无 live search 时可跑一次训练知识 compile。"""
    rec = IPRecognition(kind="known_ip", confidence=1.0, ip_name="甄嬛传", ip_type="tv")

    fallback = _pe(
        ["甄嬛", "沈眉庄", "华妃", "皇后", "苏培盛", "崔槿汐", "温实初", "流朱"],
        must_have=["甄嬛", "华妃", "皇后"],
    )
    llm = DispatchLLM(preextract=fallback)
    pack = await build_ip_knowledge_pack(rec, "loose", llm_router=llm, tavily=AsyncMock())
    ...
    assert llm.calls["preextract"] == 1
    assert llm.calls["finalize"] == 1


@pytest.mark.asyncio
async def test_strict_pipeline_stops_before_compile_when_search_has_no_citations():
    from services.ip_research_pipeline import IPResearchEvidenceError

    rec = IPRecognition(kind="known_ip", confidence=1.0, ip_name="甄嬛传", ip_type="tv")
    llm = DispatchLLM(preextract=_pe(["甄嬛"]), ground=_pe(["甄嬛"]))

    with pytest.raises(IPResearchEvidenceError, match="no citations") as exc_info:
        await build_ip_knowledge_pack(
            rec,
            "strict",
            llm_router=llm,
            tavily=AsyncMock(),
            grok_provider=FakeGrok(citations=[]),
        )

    assert exc_info.value.actual_calls == 2
    assert llm.calls["ground"] == 0
    assert llm.calls["finalize"] == 0
...
async def test_pipeline_grounds_with_grok_passages():
    ...
    pack = await build_ip_knowledge_pack(
        rec, "strict", llm_router=llm, tavily=AsyncMock(), grok_provider=grok,
    )
    assert len(grok.calls) == 2
    assert llm.calls["preextract"] == 0
    assert llm.calls["ground"] == 1
    assert llm.calls["finalize"] == 1
    assert llm.calls["missing"] == 0
...
async def test_pipeline_underfilled_raises():
    ...
    with pytest.raises(IPPackUnderfilledError):
        await build_ip_knowledge_pack(rec, "loose", llm_router=llm, tavily=AsyncMock())

...
class Settings(BaseSettings):
    ...
    oss_key_prefix: str = ""
    # OSS uploads use a signed async PUT so each attempt has a real wall-clock
    # deadline. Retrying reuses the same generated bytes and never re-runs the
    # image model.
    oss_upload_timeout_seconds: float = 30.0
    oss_upload_max_attempts: int = 2
    oss_upload_retry_backoff_seconds: float = 1.0
    oss_upload_concurrency: int = 2
    # httpx read timeout is per chunk; Live Search streams can otherwise keep a
    # broken request alive forever by sending heartbeats without useful output.
    grok_web_search_total_timeout_seconds: float = 110.0
    # Email (transactional)

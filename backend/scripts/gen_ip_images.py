"""重生成本批 IP 世界的全部图片（关 mock + jiuuij gpt-image 端点）：
世界 hero(21:9) + cover(3:2) + 可玩角色头像(2:3) + 已发布剧本封面(3:2)，写回 DB。
用法（容器内）: python -m scripts.gen_ip_images digong|orient|lianhua
"""
import asyncio
import sys
...
WORLDS = {
    "digong":  {"id": "1c92ccd5-24d8-4b73-90ae-47783b8a1128", "kind": "hybrid",   "ip_name": "大唐狄公案"},
    "orient":  {"id": "f77221c5-7cff-4185-a44f-40b4d907e84a", "kind": "known_ip", "ip_name": "东方快车谋杀案"},
    "lianhua": {"id": "c30871bc-232e-4ddf-b03b-bed4dae6966e", "kind": "hybrid",   "ip_name": "莲花楼"},
}
...
async def run(key: str) -> None:
    ...
    char_inputs = [{"name": n, "role_tag": "", "personality": p, "gender": "", "is_image_target": True}
                   for (n, p) in pnames]
    print(f"[brief:{key}] deriving cover_brief for {len(char_inputs)} chars + {len(sc_meta)} scripts...", flush=True)
    ...
    print(f"[DONE:{key}] hero={'ok' if hero else 'X'} cover={'ok' if cover else 'X'} "

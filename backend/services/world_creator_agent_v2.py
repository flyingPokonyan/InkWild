...
class WorldCreatorAgentV2:
    ...
    async def create_world(
        ...
        **kwargs: Any,
        ...
        # ---- L2 地点别名去重（union-back 之后跑，union 补进来的别名也一并合并）----
        # 确定性两条（归一化同名 / 复合名「A·B」vs「B」）+ LLM 裁决语义别名（严府 vs
        # 严嵩府邸）。合并同步改写角色引用；LLM 步非关键，失败原样保留。
        locations, _alias_renames = _dedup_location_aliases(locations, characters or [])
        try:
            locations = await self._arbitrate_location_aliases(locations, characters or [])
        except Exception as exc:  # noqa: BLE001
            logger.warning("location_alias_arbitration_failed", error=str(exc))
    ...
    async def _run_free_start_stages(
        ...
        primary_names = [
            p["name"]
            for p in candidates[: min(_FREE_START_PRIMARY_ATTEMPT_COUNT, len(candidates))]
        ]
        ...
        select_message = (
            ...
            "可玩角色：\n"
            + "\n".join(
                f"- {p['name']}（{p.get('role_tag', '')}）：{(p.get('description') or '')[:80]}"
                for p in candidates
            )
        )
        ...
        async def _work() -> list[dict]:
            # 1) 主视角优先：先强制尝试 playable 前几名，再用 LLM 弧线选择补位。
            sel = await _llm_json(_FREE_START_STAGES_SELECT_SYSTEM, select_message)
            selected_names: list[str] = []
            if isinstance(sel, dict) and isinstance(sel.get("arc_characters"), list):
                for n in sel["arc_characters"]:
                    if isinstance(n, str) and n in candidate_names and n not in selected_names:
                        selected_names.append(n)
            selected_names = selected_names[:_FREE_START_SUPPLEMENTAL_ATTEMPT_COUNT]
            attempt_names: list[str] = []
            for n in [*primary_names, *selected_names]:
                if n in candidate_names and n not in attempt_names:
                    attempt_names.append(n)

            # 2) 逐角色产出阶段；单角色失败只跳过该角色
            entries: list[dict] = []
            for name in attempt_names:
                is_primary = name in primary_names
                if len(entries) >= _FREE_START_MAX_CHARACTERS and not is_primary:
                    break
                p = next(pp for pp in candidates if pp["name"] == name)
                persona = (p.get("description") or "")[:160]
                primary_hint = (
                    "该角色位于可玩名单前列，是主视角候选。即使作品时间跨度很短，也要优先尝试"
                    "用身份 / 处境阶段组织起点；除非确实拆不出 2 档以上，才返回 has_arc=false。\n"
                    if is_primary else ""
                )
                user_message = (
                    f"世界背景：{description}\n题材：{genre}　时代：{era}\n"
                    f"角色：{name}（{p.get('role_tag', '')}）{('：' + persona) if persona else ''}\n"
                    f"可用地点：{'、'.join(loc_names) if loc_names else '（无）'}\n"
                    f"其他角色（关系可引用其名）：{'、'.join(list(roster_names)[:40])}\n\n"
                    f"{primary_hint}"
                    "判断该角色是否拥有可拆成 3–5 档的『人生 / 成长弧线』或『身份 / 处境阶段』；"
                    "若有，产出可选起点，按时间先后排序。只输出 JSON。"
                )
                try:
                    data = await _llm_json(_FREE_START_STAGES_SYSTEM, user_message)
                except Exception as exc:  # noqa: BLE001
                    logger.warning(
                        "free_start_stages_character_failed",
                        character=name, error=str(exc),
                    )
                    continue
                stages_out = _shape_stages(data, name)
                if len(stages_out) < 2 and is_primary:
                    retry_message = (
                        f"{user_message}\n\n"
                        "上一轮没有产出有效阶段。请专门按主视角的身份 / 处境变化重试："
                        "旧身份、入局时刻、调查/行动推进、危机升级、终局处境都可以作为阶段。"
                        "不要把短时间叙事误判为不能生成。只输出 JSON。"
                    )
                    try:
                        retry_data = await _llm_json(_FREE_START_STAGES_SYSTEM, retry_message)
                        stages_out = _shape_stages(retry_data, name)
                    except Exception as exc:  # noqa: BLE001
                        logger.warning(
                            "free_start_stages_primary_retry_failed",
                            character=name, error=str(exc),
                        )
                # 少于 2 档不成"选择"，该角色直接不给（单档没有选的意义）
                if len(stages_out) >= 2:
                    entries.append({"character_name": name, "stages": stages_out})
            return entries
    ...
    async def _arbitrate_location_aliases(
        ...
        data = await with_transient_retry(
            _attempt,
            max_attempts=2,
            on_retry=self._make_retry_logger("location_alias_dedup"),
        )

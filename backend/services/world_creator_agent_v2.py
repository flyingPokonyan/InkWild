...
_FREE_START_STAGES_SELECT_SYSTEM = """你在为一个互动叙事世界挑选「自由模式人生阶段」的候选角色。

从给定的可玩角色中选出**至多 3 个**拥有「贯穿全篇、阶段分明的人生 / 成长弧线」的角色——
...
_FREE_START_STAGES_SYSTEM = """你在为一个互动叙事世界设计「自由模式起点选择」——让玩家挑某个角色人生的哪个阶段开局。

只有当该角色拥有**贯穿全篇、阶段分明**的人生 / 成长弧线时才产出阶段，典型如：
- 修仙境界（炼气 → 筑基 → 元婴…）
- 宫廷位份（常在 → 妃 → 皇太后）
- 力量体系序列 / 段位（序列9 → 序列0）
- 年龄 / 事业成长（少年 → 成名 → 巅峰）

若该角色只处于**单一时间切片**（一次探案 / 一场事件 / 短篇），返回 {"has_arc": false, "stages": []}。
...
class WorldCreatorAgentV2:
    ...
    async def _run_free_start_stages(
        ...
        """为拥有贯穿全篇、阶段分明人生 / 成长弧线（修仙境界 · 宫廷位份 · 官职升迁 ·
        序列途径 · 年龄成长…）的可玩角色产出自由模式起点选择。一个世界可有多个弧线
        角色各配一套阶段（群像正剧没有单一"主角"，2026-07-02 由单主角扩展）：先一次
        轻量 LLM 挑选至多 3 个弧线角色，再逐角色产出阶段。一个都没有则留空 →
        world.free_start_stages 保持 None，自由模式走固定 initial_location 老开局。

        非关键阶段：任何失败（含单个角色的阶段生成失败）都静默跳过（不 warn、不阻塞
        后续），起点选择只是增益。产出用**角色本名**记档（UUID 在 publish/apply 阶段
        解析），与 known_relations 的 npc 名一致，全程 name-based 才能跨 publish 稳定。
        """
        start = time.monotonic()
        yield progress_event(
        ...
        async def _work() -> list[dict]:
            ...
            if isinstance(sel, dict) and isinstance(sel.get("arc_characters"), list):
                for n in sel["arc_characters"]:
                    if isinstance(n, str) and n in candidate_names and n not in arc_names:
                        arc_names.append(n)
            arc_names = arc_names[:3]
            ...
            for name in arc_names:
                ...
                user_message = (
                    ...
                    "判断该角色是否拥有『贯穿全篇、阶段分明』的人生 / 成长弧线；若有，产出 3–5 个"

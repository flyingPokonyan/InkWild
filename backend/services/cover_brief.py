...
# Quality + single-subject + anti-collage —— 把"该统一的"留住，"该按世界变的"交给模型。
#
# 演进史：2026-06-15 "电影剧照/像一帧镜头"(太平太截图) → 2026-06-23 "影视级海报/电影感写实
# +戏剧光影+景深+高级感"。后者质量上去了，但把**媒介锁死成写实电影大片**——所有世界(宫廷/
# 仙侠/恐怖/赛博)都撞成同一种亮面写实海报，模型给不了水墨/工笔/阴翳插画。用户反馈"生图都
# 一个风格"，根因正是这条把画风/光线/色调一并钉死。
#
# 现版本(2026-06-23 晚)：只保留不该变的——代表作级质量、单一主体、反 montage、构图讲究；
# **媒介/画风/色调/光线交还给模型按作品本身气质决定**(写实电影感、插画、水墨、工笔、油画
# 皆可)。商业垃圾交给 _LOGO_NEGATIVE 兜。
_CINEMATIC_COHESION = (
    "画面要精致考究、构图讲究、有明确的单一主体，达到代表作级别的视觉质量。"
    "媒介、画风、色调与光线由作品本身的气质决定——写实电影质感、插画、水墨、工笔、油画皆可，"
    "不必千篇一律。保持单一、浑然一体的画面，不要把多个标志性元素拼贴、分屏或堆叠成大杂烩。"
)
...
# 2026-06-23 晚：撤掉之前"清新亮色"的强推(那是另一条同质化指令)——色调明暗交给模型按作品
# 气质定，这里只保留"主体清晰可辨、别糊进死黑看不清"这条 legibility 地板。
_DARK_UI_HINT = "画面会在深色网站上陈列，主体需清晰可辨、不要糊进死黑（具体色调明暗由作品气质决定）。"

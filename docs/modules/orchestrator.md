...
| **v2 NPC 块预取批量化**：reflection / voice_anchor / peer_relations 各一次 set 查询（消除 `_npc_block_when_ready` 循环里 3×N 串行往返；`db` 是共享 AsyncSession 不能并行 gather，只能批量） | ✅ | `batch_get_reflections` / `batch_get_npc_recent_utterances` / `batch_get_npc_peer_relations`（与 `batch_query_npc_memories` 同范式） |

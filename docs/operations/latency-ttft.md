...
| `hlm-free-20260612T030743Z` | `f33e421`(C-2 裁clues + compression/reflection 关思考) | OpenCode 全新红楼梦30轮 | **③narrator 反套路✅✅脱离 anchoring 坐实有效**:「不是X是Y」94→**3%**、开篇 55→**3%**、破折号 6.4→**1.7**(印证旧轮 89% 是同 session anchoring 污染,prompt 本身有效);**①compression/reflection CoT 泄漏归零**(reasoning_content_observed=0);**②parse 信号全 0**(短局未入退化区,验 C-2 无新问题);director 缓存 48.7→**59.9%**(C-2 裁尾部副作用);IP 忠实强(器物/地点/人物全对)/能动性好/0 错误;TTFT p50 18.5s(全新短局,不可比长局) |
| `hp-free-continue-20260612T055041Z` | `f33e421`(C-2) 同 session cafa4df5 续 R121-140 | OpenCode | **C-2 合力量化✅(同 session A/B,唯一变量 C-2,按时间窗口精确分段)**:director input avg **39840→29157**(-27%/-10.7K,吻合预期 9.8K)、max **44856→31734**(正好压到退化阈值 32K 以下)→ **空白退化 6 次/20轮→0 次**、parse_failure_final 2→**0**、穿透玩家 0;TTFT p95 **51→26s**(无退化重试尖峰)、avg 27.2→20.8s;reasoning 归零;0 错误。**parse 线闭环**=C-2 预防层(降基线扼杀源头)+ 熔断/升级止损层(本轮因 0 发作未触发) |
| `hlm-free-20260612T064050Z` | `f33e421` 全优化在场 OpenCode 全新红楼梦 **60轮** | **★完整基线(动架构前的基准线)**:TTFT p50 R1-30 **17.3→R31-60 23.5s**(稳态~23s,decode 地板仍在,**非 10-15s**)、done p50 25→33s;**C-2 验证:director input max 60轮才 29.7K<32K 退化阈值→全程 0 退化/0 CoT 泄漏/0 错误**;缓存 director 54.8/narrator 70/npc 50.2%;句式「不是X是Y」**15%**(vs HP 94%,优;60轮>30轮3% 因后半部悲剧情感转折密集);成本~1305 分≈13 元/60轮(官方价记账,OpenCode 实际免费)。**结论:成本/parse/CoT/文风/长局稳定五线全收口,唯 TTFT decode 地板未动(~23s),要 10-15s 须架构改(narrator 不等 NPC / director 关键路径轻量化)** |
